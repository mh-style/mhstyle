 ...
 * Licensed under: SEE LICENSE IN https://github.com/mhSlider2/mhSlider2/blob/master/LICENSE
 */
/**
 * mh slider
 * @version 2.3.4
...
;(function($, window, document, undefined) {
	 ...
	 *                    if you know the slider will always be visible you can set `checkVisibility` to `false` to
	 ...
	 * @public
	 * @param {mh.Width} [dimension=mh.Width.Default] - The dimension to return.
	...
	/**
	 * Refreshes the slider primarily for adaptive purposes.
	 * @public
	 */
	...
	mh.prototype.registerEventHandlers = function() {
		...
		if (this.settings.mouseDrag) {
			...
			this.$stage.on('dragstart.mh.core selectstart.mh.core', function() { return false });
		}
	...
	mh.prototype.onDragEnd = function(event) {
		...
		if (delta.x !== 0 && this.is('dragging') || !this.is('valid')) {
			...
			if (Math.abs(delta.x) > 3 || new Date().getTime() - this._drag.time > 300) {
				this._drag.target.one('click.mh.core', function() { return false; });
			}
		}
	...
	mh.prototype.onTransitionEnd = function(event) {
		...
		if (event !== undefined) {
			...
			// Catch only mh-stage transitionEnd event
	 ...
	 * @param {String} [namespace=slider] - The event namespace.
	...
	mh.prototype.trigger = function(name, data, namespace, state, enter) {
		...
		if (!this._supress[name]) {
			...
			this.register({ type: mh.Type.Event, name: name });
	...
	/**
	 * The jQuery Plugin for the mh slider
		...
		return this.each(function() {
			...
			if (!data) {
				...
				$.each([
					...
					data.register({ type: mh.Type.Event, name: event });
			...
			'initialized.mh.slider change.mh.slider resized.mh.slider': $.proxy(function(e) {
				...
				if ((e.property && e.property.name == 'position') || e.type == 'initialized') {
					...
					if (settings.lazyLoadEager > 0) {
						...
						// If the slider is looping also preload images that are to the "left"
						if (settings.loop) {
...
;(function($, window, document, undefined) {
	...
	var AutoHeight = function(slider) {
		...
		// Autoresize the height of the slider when window is resized
		// When slider has images, the height is dependent on the width
...
;(function($, window, document, undefined) {
	...
	Video.prototype.thumbnail = function(target, video) {
		...
		// wrap video content into mh-video-wrapper div
		target.wrap( $( '<div/>', {
...
;(function($, window, document, undefined) {
		...
		this._handlers = {
			'changed.mh.slider': $.proxy(function(e) {
				if (e.namespace && e.property.name === 'settings') {
					...
					// of the slider was changed through user interaction.
					this._time = 0;
				}
			}, this),
			'initialized.mh.slider': $.proxy(function(e) {
	...
	Autoplay.prototype.play = function(timeout, speed) {
		...
		// Calculate the elapsed time since the last transition. If the slider
...
;(function($, window, document, undefined) {
	...
	var Navigation = function(slider) {
		...
		/**
		 * Overridden methods of the slider.
		 * @protected
		 * @type {Object}
		 */
		this._overrides = {
	...
	/**
	 * Initializes the layout of the plugin and extends the slider.
	 * @protected
	 */
	Navigation.prototype.initialize = function() {
		...
		// override public methods of the slider
	...
	/**
	 * Gets the current page position of the slider.
	 * @protected
	 * @returns {Number}
	 */
